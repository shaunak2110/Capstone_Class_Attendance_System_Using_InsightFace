...
function InfoRow({ icon: Icon, label, value }) {
  return (
    <div className="flex items-start gap-4 p-4 rounded-xl bg-white dark:bg-black/20 border border-slate-200 dark:border-white/5 hover:bg-slate-100 dark:hover:bg-white/5 transition-colors">
  ...
  return (
    <div className="relative min-h-[calc(100vh-2rem)] bg-slate-50 dark:bg-[#0f1117] text-slate-800 dark:text-slate-200 p-4 sm:p-8 rounded-xl overflow-hidden font-sans shadow-2xl selection:bg-purple-500/30">
      ...
      <div className="relative space-y-6 animate-in fade-in duration-700 z-10 w-full px-4 sm:px-6 lg:px-10">        {/* Header */}
        <div className="flex items-center justify-between pb-6 border-b border-slate-300 dark:border-white/10">
          ...
          <Button variant="outline" onClick={handleBack} className="bg-white dark:bg-white/5 hover:bg-slate-200 dark:hover:bg-white/10 text-slate-900 dark:text-white border-slate-300 dark:border-white/10 shadow-lg backdrop-blur-md transition-all duration-300">
            <ArrowLeft className="mr-2 h-4 w-4" /> Back
          </Button>
        </div>

        {loading && (
          <Card className="bg-white dark:bg-white/5 border-slate-300 dark:border-white/10 backdrop-blur-xl shadow-xl">
            <CardContent className="p-10 text-center text-slate-600 dark:text-slate-400">Loading profile…</CardContent>
          </Card>
        )}
        ...
        {!loading && profile && (
          <>
            {/* Identity Card */}
            <Card className="bg-white dark:bg-white/5 border-slate-300 dark:border-white/10 backdrop-blur-xl shadow-xl overflow-hidden">
              ...
              <CardContent className="px-6 pb-6 -mt-12 relative z-10">
                <div className="flex flex-col sm:flex-row items-start sm:items-end gap-4">
                  <Avatar className="h-28 w-28 border-4 border-[#0f1117] shadow-xl bg-gradient-to-br from-indigo-500 to-purple-600">
                    <AvatarFallback className="text-slate-900 dark:text-white text-3xl font-bold">{initials}</AvatarFallback>
                  </Avatar>
                  <div className="flex-1 pb-1">
                    <h3 className="text-2xl font-bold text-slate-900 dark:text-white">{profile.name}</h3>
                    <p className="text-slate-600 dark:text-slate-400 text-sm mt-1">@{profile.username}</p>
            ...
            {/* Details Card */}
            <Card className="bg-white dark:bg-white/5 border-slate-300 dark:border-white/10 backdrop-blur-xl shadow-xl mt-6">
              <CardHeader className="border-b border-slate-200 dark:border-white/5 pb-4">
                <CardTitle className="text-lg text-slate-900 dark:text-white font-semibold">Account Details</CardTitle>
              </CardHeader>
